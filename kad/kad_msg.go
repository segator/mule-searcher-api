...
type Kademlia2SearchKeyReq struct {
	contactID ID
	ip        uint32
	udpPort   uint16
	version   uint8
...
func (m *Kademlia2SearchKeyReq) set(pPacket *Packet) {
	m.ip = pPacket.ip
	m.udpPort = pPacket.port

	bi := ByteIO{buf: pPacket.buf}
	bi.readBytesFast(m.keyReqHash.getHash())
	bi.readUint16() //starpoisiotn
	searchType := bi.readUint8()
	if searchType == 0x00 {

	}else if searchType == 0x01 {
		rest := len(bi.buf)-bi.pos
		query := string(bi.readBytes(rest))
		println(query)
	}
